...
/* We are called once per section from amiga_symfile_read.  We
...
#if 0
		      if (amiga_make_msymbol_special_p)
			gdbarch_amiga_make_msymbol_special (gdbarch,
							  sym, mtramp);
#endif
		    }
		}
...
/* Try to find the target resolved function entry address of a STT_GNU_IFUNC
   ...
   amiga_gnu_ifunc_resolve_by_cache must have been already called for NAME to
			  ...
			  const struct elfinfo *ei)
{
  ...
  if (storage_needed > 0)
	...
	error (_("Can't read symbols from %s: %s"),
      ...
      amiga_symtab_read (reader, objfile, ST_REGULAR, symcount, symbol_table,
		       false);
    }
  ...
  if (storage_needed > 0)
    {
	 ...
	 It happens only in the case when amiga_slurp_reloc_table sees
	 asection->relocation NULL.  Determining which section is asection is
	 done by _bfd_amiga_get_synthetic_symtab which is all a bfd
	...
	error (_("Can't read symbols from %s: %s"),
      ...
      amiga_symtab_read (reader, objfile, ST_DYNAMIC, dynsymcount,
		       dyn_symbol_table, false);

      amiga_rel_plt_read (reader, objfile, dyn_symbol_table);
    }
  ...
  if (synthcount > 0)
	...
	synth_symbol_table[i] = synthsyms + i;
      amiga_symtab_read (reader, objfile, ST_SYNTHETIC, synthcount,
...
amiga_symfile_read (struct objfile *objfile, symfile_add_flags symfile_flags)
{
  ...
  if (!(objfile->flags & OBJF_READNEVER))
    bfd_map_over_sections (abfd, amiga_locate_sections, (void *) & ei);
  ...
  if (ei.mdebugsect)
    {
      ...
      /* .mdebug section, presumably holding ECOFF debugging
         information.  */
      swap = get_amiga_backend_data (abfd)->amiga_backend_ecoff_debug_swap;
      if (swap)
  ...
  if (dwarf2_has_info (objfile, NULL))
    {
      ...
      /* amiga_sym_fns_gdb_index cannot handle simultaneous non-DWARF
	  ...
	  && dwarf2_initialize_objfile (objfile, &index_kind))
	{
	  switch (index_kind)
	    ...
	    case dw_index_kind::GDB_INDEX:
	      objfile_set_sym_fns (objfile, &amiga_sym_fns_gdb_index);
	      break;
	    case dw_index_kind::DEBUG_NAMES:
	      objfile_set_sym_fns (objfile, &amiga_sym_fns_debug_names);
	      break;
	    }
	}
      else
	{
	  /* It is ok to do this even if the stabs reader made some
	     ...
	     when needed.  */
	  objfile_set_sym_fns (objfile, &amiga_sym_fns_lazy_psyms);
	}
    }
...
/* The same as amiga_sym_fns, but not registered and lazily reads
   psymbols.  */

const struct sym_fns amiga_sym_fns_lazy_psyms =
{
  amiga_new_init,			/* init anything gbl to entire symtab */
  amiga_symfile_init,		/* read initial info, setup for sym_read() */
  amiga_symfile_read,		/* read a symbol file into symtab */
...
/* The same as amiga_sym_fns, but not registered and uses the
   DWARF-specific GNU index rather than psymtab.  */
const struct sym_fns amiga_sym_fns_gdb_index =
{
  amiga_new_init,			/* init anything gbl to entire symab */
  amiga_symfile_init,		/* read initial info, setup for sym_red() */
  amiga_symfile_read,		/* read a symbol file into symtab */
  NULL,				/* sym_read_psymbols */
  amiga_symfile_finish,		/* finished with file, cleanup */
...
const struct sym_fns amiga_sym_fns_debug_names =
{
  amiga_new_init,			/* init anything gbl to entire symab */
  amiga_symfile_init,		/* read initial info, setup for sym_red() */
  amiga_symfile_read,		/* read a symbol file into symtab */
